...
<script>
import { prepareBoxplotData } from '../../utils/dataTool'

export default {
    props:['boxPlotData', 'boxPlotY', 'boxPlotTitle'],
    data () {
        return {
            echarts:'',
            nData: []
    ...
    watch: {
        boxPlotData(val) {
            var self = this
            //监听到数据变化
            console.log('监听到数据变化', 'boxPlotData', self.boxPlotData, 'boxPlotY', self.boxPlotY)
            self.boxPlotData.forEach(v => {
                self.nData.push(v[self.boxPlotY])
            })
    ...
    methods: {
        echartsInit () {
            let self = this
            var data = prepareBoxplotData([self.nData])
