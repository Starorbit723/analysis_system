<template>
	<div>
		<div class="histogram_content" ref="histogram"></div>
  		<div class="histogram_content2" ref="histogram2"></div>
	</div>
</template>

<script>
export default {
  	props:['histogramData', 'histogramX', 'histogramTitle'],
  	data () {
    	return {
			echarts:'',
			echarts2:'',
			nData: [],
			step:10,
			XArr: [],
			arrForY:[],
			YArr:[]
    	}
  	},
  	watch: {
        histogramData(val) {
            var self = this
            //监听到数据变化
			console.log('监听到数据变化', 'histogramData', self.histogramData, 'histogramX', self.histogramX)
			self.doX()
            self.histogramData.forEach(v => {
                let tempArr = []
                tempArr.push(v.data)
                tempArr.push(v.frequency)
                self.nData.push(tempArr)
            })
            console.log('nData:', self.nData)
            self.echartsInit2()
        }
    },
  	methods: {
		//处理X轴
		doX () {
			var self = this
			let min = self.histogramData[0].data
			let max = self.histogramData[self.histogramData.length - 1].data
			let chaStep = parseInt((max - min) / self.step)
			var temp = min
			for (let i = 0; i < self.step; i++) {
				let str = temp + '~' + (temp + chaStep)
				temp += chaStep
				self.XArr.push(str)
				self.arrForY.push(temp)
			}
			console.log('X轴', self.XArr, self.arrForY)
			self.doY()
		},
		//处理Y轴
		doY	() {
			var self = this
			let tempArr = []
			for (let m = 0; m < self.step; m++) {
				tempArr[m] = 0
			}
			self.histogramData.forEach(v => {
				for (let i = 0; i < self.step; i++) {
					if (v.data <= self.arrForY[i]) {
						tempArr[i] += 1
						return
					}
				}
			})
			console.log('Y轴', tempArr)
			self.YArr = tempArr
			self.echartsInit()	
		},
    	echartsInit () {
			let self = this
			let initEcharts = self.$refs.histogram
			self.echarts = self.$echarts.init(initEcharts)
			self.echarts.setOption({
					title: {
						text: self.histogramTitle,
						left:'center',
						textStyle:{
							fontSize: '20',
							color: '#266099'    
					}
				},
				xAxis: {
					type: 'category',
					nameGap:5,
					name: self.histogramX,
					data: self.XArr,
					axisLabel :{
						interval:0,
						rotate:20
					}
				},
				yAxis: {
					type: 'value'
				},
				series: [{
					type: 'bar',
						data: self.YArr
					}],
        	toolbox: {
					show: true,
					itemSize:20,
					feature: {
						saveAsImage: {
							show:true,
							icon:'image://data:image/png;base64,',
							excludeComponents :['toolbox'],
							pixelRatio: 2,
							title:'Save'
            			}
          			}
        		}
      		})
		},
		echartsInit2 () {
			let self = this
			let initEcharts = self.$refs.histogram2
			self.echarts2 = self.$echarts.init(initEcharts)
			self.echarts2.setOption({
					title: {
						text: self.histogramTitle,
						left:'center',
						textStyle:{
							fontSize: '20',
							color: '#266099'    
					}
				},
				xAxis: {
					type: 'value',
					nameGap:5,
					name: self.histogramX
				},
				yAxis: {
					type: 'value'
				},
				series: [{
					type: 'scatter',
                    data: self.nData
				}],
        		toolbox: {
					show: true,
					itemSize:20,
					feature: {
						saveAsImage: {
							show:true,
							icon:'image://data:image/png;base64,',
							excludeComponents :['toolbox'],
							pixelRatio: 2,
							title:'Save'
            			}
          			}
        		}
      		})
		}
		
  	}
}
</script>

<style>
  .histogram_content{
    width:800px;
    height: 500px;
    margin: 0 auto;
  }
  .histogram_content2{
	width:800px;
    height: 500px;
    margin: 50px auto;
  }
</style>
